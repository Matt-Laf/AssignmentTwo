export const dataPage1 = [
  {
    id: 1,
    title: "Artist",
    content: `Diego Riviera was a Mexican Muralist (heavily inspired
            by socialist realism) responsible for the painting “Man, Controller
            of the Universe (1934)”; a mural reflecting the historically
            defining progress mankind was making politically and technologically
            in that era. The painting has a rich history that presents us with a
            glimpse into the entangled relationship between art and politics.
            The piece was originally commissioned by Nelson Rockerfeller for the
            Rockerfeller center in New York City. However, after it was
            complete, the inclusion of Vladimir Lenin was seen as controversial;
            a consequence of the first red scare in America. Riviera’s refusal
            to remove the depiction of Lenin was seen as “un-American” by
            Rockerfeller, who had the piece destroyed. Riviera repainted the
            piece, which is now displayed in the Palace of Fine Arts in Mexico
            City.`,
    links: [
      {
        phrase: "Mexican Muralist",
        url: "https://www.moma.org/collection/terms/mexican-muralism",
      },
    ],
  },
  {
    id: 2,
    title: "Description",
    content: `It was completed in the fresco medium; water based paint applied
            directly to plaster on a 480 cm × 1145 cm canvas. The painting is
            divided into 2 halves, the left side depicting bourgeois society and
            military, and the right side depicting revolutionary protesters. In
            the center of the painting is a man holding a controller while
            standing under a huge machine, next to a microscope, above a pipe
            with growths of vegetation, and behind a large hand holding a
            mechanical sphere. Cascades of light burst forth from the sphere,
            each containing a depiction of the cosmos or of microscopic
            organisms. This is a direct reference to the title of the work and a
            testament to Riviera’s socialist beliefs; an everyday laborer is
            depicted as being the controller of the universe and driver of
            history while surrounded by mankind's achievements bursting forth
            from machines. The purpose of Mexican Muralism was to build
            community by communicating social and political messages through
            art. The political message within this particular piece was so
            strong that it resulted in censorship; the piece had become a real
            representation of the clash between revolutionary forces and
            reactionaries. Riviera incorporated symmetrical balance, rhythm, and
            shape to draw the viewer to the center. These principles also help
            in establishing the juxtaposing halves of the painting.`,
    links: [
      {
        phrase: "fresco",
        url: "https://upload.wikimedia.org/wikipedia/commons/thumb/c/c6/Murales_Rivera_-_Markt_in_Tlatelolco_3.jpg/800px-Murales_Rivera_-_Markt_in_Tlatelolco_3.jpg",
      },
    ],
  },
  {
    id: 3,
    title: "Critical Question",
    content: `How does Riviera’s use of technology, class, and political figures challenge or reinforce your own conception of who holds power in shaping history? How does this reflection influence your own perspective on modern power structures?`,
    links: [{}],
  },
];

export const imgsPage1 = [
  {
    src: "https://upload.wikimedia.org/wikipedia/commons/9/93/Libro_Los_Viejos_Abuelos_Foto_68.png",
    caption: "Man at the Crossroads, Diego Rivera",
  },
  {
    src: "https://www.moma.org/wp/inside_out/wp-content/uploads/2012/03/Rock-center-mural.sm_.jpg",
    caption: "Man at the Crossroads, Diego Rivera, Rockefeller Center",
  },
  {
    src: "https://smarthistory.org/wp-content/uploads/2023/01/25963100640_6e4cf2d518_k.jpg",
    caption: "Man at the Crossroads, Diego Rivera",
  },
  {
    src: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRwLSm7dxE4oDIWHLlEY-aT_WGo2wasYhGbeg&s",
    caption: "Diego Rivera",
  },
];

export const dataPage2 = [
  {
    id: 1,
    title: "Artist",
    content: `Jean-Michel Basquiat was a NYC based neo-expressionist painter who emerged from the street art and graffiti movement. He is responsible for painting “Obnoxious Liberals”(1982).`,
    links: [
      {
        phrase: "neo-expressionist",
        url: "https://en.wikipedia.org/wiki/Neo-expressionism",
      },
    ],
  },
  {
    id: 2,
    title: "Description",
    content: `The painting gives a glimpse into the mind of an artist from the Haitian diaspora growing up in Reagan era America, full of racial tensions, gentrification, and the widespread application of neoliberal policies and privatization. 
This painting is now held in the Art Gallery of Ontario, Toronto, while many of the artist's other works are in the MoMA. It is painted with oilstick, spray, and acrylic on a 172.72 x 259.08 cm canvas. 
The painting depicts 3 figures chaotically scribbled onto the page. The left figure shows a character bound by chains and bars in an asbestos filled room. The 2 figures on the right are donned in iconic American garb (top hats) and surrounded by dollar signs; the message in the middle of the canvas reads "not for sale”. 
Basquiat used color, space, and movement to guide the eye through a chaotic and visceral piece. Contrast between red, blue, and white emphasizes the forms while also representing America. The black figure represents the sequestering and imprisonment of black americans. He was not shy about using written words in his work, which brought forth additional clarity and harkens back to his roots as a graffiti artist. 
This painting is a reflection of Basquiat's own frustration with capitalist co-option of the art industry, which reduced artwork to mere commodities (which is ironic given the record-breaking auction prices his work fetched). 
`,
    links: [
      {
        phrase: "asbestos",
        url: "https://en.wikipedia.org/wiki/Mexican_muralism",
      },
    ],
  },
  {
    id: 3,
    title: "Critical Question",
    content: `How does Basquiat's chaotic composition help or detract from his critiques of capitalism? How does this work to shape your own understanding of art as a commodity? `,
    links: [{}],
  },
];

export const imgsPage2 = [
  {
    src: "https://www.thebroad.org/sites/default/files/styles/webp_convert_only/public/art/basquiat-obnoxious_liberals.jpg.webp?itok=bduIlZ3a",
    caption: "Obnoxious Liberals, Jean-Michel Basquiat",
  },
  {
    src: "https://pictures.abebooks.com/inventory/31950224513.jpg",
    caption: "Jean-Michel Basquiat",
  },
];

export const dataPage3 = [
  {
    id: 1,
    title: "Artist",
    content: `This painting was created by Lenardo da Vinci created between 1495 to 1498 during the Italian Renaissance.`,
    links: [
      {
        phrase: "Italian Renaissance",
        url: "https://en.wikipedia.org/wiki/Mexican_muralism",
      },
    ],
  },
  {
    id: 2,
    title: "Description",
    content: `This painting was created to demonstrate the twelve disciples' confusion and uneasiness when they found out that one of them was going to betray Jesus. Lenardo da Vinci carefully captured each of the disciples' emotions with the painting that already tells us a story by just looking at it.  This painting was commissioned by the Duke of Milan (another word to often used to refer to as the emperor or ruler of Milan, a city in Italy) named Ludovico Sforza. Ludovico loved art and was rich and powerful so he paid Lenerado to paint this painting. The Last Supper is a mural painting (a type of artwork that is painted or applied directly on to a wall or ceiling and cannot be moved). He painted it using tempera (which is a painting technique that mixes pigments things like water and egg yolk) and oil paint on a dry wall. The style of the painting is High Renaissance which was a period in art where artists create realistic and detailed art, to show emotions and movement. The genre of the painting is religious as it used to represent a biblical time. 


The museum that to this day still has the painting is in the Convent of Santa Maria delle Grazie in Milan, Italy. (This is a well known church located in the heart of the city, Milan visited by many tourists. Built in 1469 and finished in 1482)
`,
    links: [
      {
        phrase: "fresco",
        url: "https://en.wikipedia.org/wiki/Mexican_muralism",
      },
    ],
  },
  {
    id: 3,
    title: "Critical Question",
    content: `In the Last Supper painting by Leonardo da Vinci, he uses art to describe the reactions the disciples had to Jesus’s news. How does this demonstrate to us how art can be used to tell a story? Think of another art piece that conveyed such strong emotion and a story to you?`,
    links: [{}],
  },
];

export const imgsPage3 = [
  {
    src: "https://upload.wikimedia.org/wikipedia/commons/thumb/4/48/The_Last_Supper_-_Leonardo_Da_Vinci_-_High_Resolution_32x16.jpg/1200px-The_Last_Supper_-_Leonardo_Da_Vinci_-_High_Resolution_32x16.jpg",
    caption: "The Last Supper, Leonardo Da Vinci",
  },
  {
    src: "https://hips.hearstapps.com/hmg-prod/images/portrait-of-leonardo-da-vinci-1452-1519-getty.jpg?crop=1xw:1.0xh;center,top&resize=640:*",
    caption: "Leonardo Da Vinci",
  },
  {
    src: "https://upload.wikimedia.org/wikipedia/commons/8/83/Santa_Maria_delle_Grazie_Milan_2013.jpg",
    caption: "Basilica di Santa Maria delle Grazie",
  },
];

export const dataPage4 = [
  {
    id: 1,
    title: "Artist",
    content: `This painting was created by Alexander Young Jackson also known as A.Y. Jackson in his Toronto studio in November 1914. He is a famous artist and a part of the Group of Seven which is a group of Canadian landscape painters. `,
    links: [
      {
        phrase: "Group of Seven",
        url: "https://en.wikipedia.org/wiki/Mexican_muralism",
      },
    ],
  },
  {
    id: 2,
    title: "Description",
    content: `He created this painting based on a natural sketch created along the Oxtongue River in Algonquin Park. It was not a commissioned work and he was just inspired to create this based on the Candians autumn outdoor wilderness and landscape. It also showcases the red maple leaf which is a major Canadian symbol that is known to represent the canadian identity. 

A.Y. Jackson has a Post-Impressionist style which is a way of painting that tries to portray more feeling, movement  and emotion in the painting by using bright and bold colours(like blue and red), thick brushstroke (show movement of the waves in the lake and the way the wind blows the leaves)  and shapes that are over exaggerated. This painting is created by using oil mixed with coloured pigments on a canvas. The genre of the painting is landscape as it portrays an image of the outdoors. 

This painting can be found in the National Gallery of Canada art museum in Ottawa. (Built in 1988 and holds many indigenous and Canadian artwork in the world). 
`,
    links: [
      {
        phrase: "Post-Impressionist",
        url: "https://en.wikipedia.org/wiki/Mexican_muralism",
      },
    ],
  },
  {
    id: 3,
    title: "Critical Question",
    content: `How does the Red Maple painting represent Canadian culture? Do you think this is a good representation of Canadian landscapes? If not, what would you change?`,
    links: [{}],
  },
];

export const imgsPage4 = [
  {
    src: "https://www.artcountrycanada.com/images/jackson-a-y-The-Red-Maple-good.jpg",
    caption: "The Red Maple, A.Y. Jackson",
  },
  {
    src: "https://thegroupofseven.ca/wp-content/uploads/2019/01/a-y-jackson.jpg",
    caption: "Alexander Young Jackson",
  },
  {
    src: "https://ottawatourism.ca/sites/default/files/styles/facebook_share/public/images/2020/02/National-Gallery-of-Canada-exterior-044A4695-credit-Ottawa-Tourism.jpg?itok=4pTZ32T0",
    caption: "National Gallery of Canada",
  },
];

export const dataPage5 = [
  {
    id: 1,
    title: "Artist",
    content: `Jacques-Louis David (1748–1825) was a French Neoclassical painter known for his dramatic, history-based works that emphasized moral virtue and civic duty. His most famous paintings, such as The Oath of the Horatii and Napoleon Crossing the Alps, combine classical themes with modern political and social contexts. As a key figure during the French Revolution and under Napoleon, David's art often served as both a reflection of and propaganda for the era's ideals. His precise style and impactful compositions influenced generations of artists and left a lasting legacy in Western art.`,
    links: [
      {
        phrase: "Neoclassical",
        url: "https://en.wikipedia.org/wiki/Mexican_muralism",
      },
    ],
  },
  {
    id: 2,
    title: "Description",
    content: `The Oath of the Horatii (1784) is one of Jacques-Louis David's most iconic paintings, exemplifying the Neoclassical style. The painting depicts a pivotal moment from ancient Roman history, where three brothers from the Horatii family swear an oath to defend their city, Rome, against their enemies, the Curatii. With their swords raised, they make a solemn vow in front of their father, symbolizing honor, sacrifice, and patriotism.
The composition emphasizes strong lines, dramatic lighting, and a sense of stoic resolve, which reflect the ideals of the Enlightenment, such as duty, virtue, and civic responsibility. The contrasting emotions of the brothers' determination and the sorrow of their women highlight the tension between personal sacrifice and familial bonds. The painting became a powerful symbol of the French Revolution’s ideals, promoting the notion of collective duty over individual desire.

`,
    links: [
      {
        phrase: "French Revolution",
        url: "https://en.wikipedia.org/wiki/Mexican_muralism",
      },
    ],
  },
  {
    id: 3,
    title: "Critical Question",
    content: `How does David communicate values of loyalty, sacrifice, and patriotism through his work? Think of this in the context of revoluntionary France.`,
    links: [{}],
  },
];

export const imgsPage5 = [
  {
    src: "https://upload.wikimedia.org/wikipedia/commons/thumb/3/35/Jacques-Louis_David%2C_Le_Serment_des_Horaces.jpg/1280px-Jacques-Louis_David%2C_Le_Serment_des_Horaces.jpg",
    caption: "Oath of the Horatii, Jacque-Louis David",
  },
  {
    src: "https://upload.wikimedia.org/wikipedia/commons/thumb/c/c6/David_Self_Portrait.jpg/800px-David_Self_Portrait.jpg",
    caption: "Jaques-Louis David",
  },
  {
    src: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/66/Louvre_Museum_Wikimedia_Commons.jpg/800px-Louvre_Museum_Wikimedia_Commons.jpg",
    caption: "Louvre Museum",
  },
];

export default { dataPage1, dataPage2, dataPage3, dataPage4 };
